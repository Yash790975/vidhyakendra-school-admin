...
import { studentsApi, type Student, type StudentContact } from '@/lib/api/students'
...
export default function InactiveStudentsPage() {
  const [rows, setRows]                       = useState<InactiveStudentRow[]>([])
  ...
  const fetchInactiveStudents = useCallback(async () => {
    ...
    try {
      ...
      const [inactiveRes, blockedRes, archivedRes] = await Promise.allSettled([
        studentsApi.getAll({ status: 'inactive', ...(instituteId && { institute_id: instituteId }), student_type: studentType }),
        studentsApi.getAll({ status: 'blocked',  ...(instituteId && { institute_id: instituteId }), student_type: studentType }),
        studentsApi.getAll({ status: 'archived', ...(instituteId && { institute_id: instituteId }), student_type: studentType }),
      ])
      ...
      const enriched = await Promise.all(
        studentList.map(async (student): Promise<InactiveStudentRow> => {
          let contact: StudentContact | null = null
          try {
            const contactRes = await studentsApi.getPrimaryContactByStudent(student._id)
            contact = (contactRes as any)?.result ?? null
          } catch {
            // contact not found — non-fatal
          }
      ...
      console.error('[InactiveStudents] Unexpected error:', err instanceof Error ? err.message : err)
      setError('Something went wrong while loading students. Please refresh the page.')
    } finally {
      setLoading(false)
    }
  }, [])

  useEffect(() => { fetchInactiveStudents() }, [fetchInactiveStudents])
  ...
  const confirmReactivate = async () => {
    ...
    setActionError('')
    try {
      const res = await studentsApi.update(reactivateId, { status: 'active' })
      if (!res.success) {
        setActionError('Could not reactivate this student. Please try again.')
        return
      }
      setRows(prev => prev.filter(r => r.student._id !== reactivateId))
      setReactivateId(null)
    } catch (err: unknown) {
      console.error('[InactiveStudents] Reactivate error:', err instanceof Error ? err.message : err)
  ...
  const filteredRows = rows.filter(row => {
    ...
    const matchSearch = !q || (
      ...
      row.student.student_code?.toLowerCase().includes(q) ||
      row.contact?.email?.toLowerCase().includes(q) ||
      row.contact?.mobile?.includes(q)
    )
    const matchStatus = statusFilter === 'all' || row.student.status === statusFilter
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <p className="mt-4 text-sm text-muted-foreground">Loading inactive students...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
        ...
        <Card className="border-2 border-red-200 bg-red-50">
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <Button size="sm" onClick={fetchInactiveStudents} className="ml-auto shrink-0">
                Retry
              </Button>
            </div>
          </CardContent>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1 text-sm sm:text-base">
            View and reactivate deactivated student records
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid gap-3 sm:gap-4 grid-cols-2 xl:grid-cols-4">
        {[
          { label: 'Total',    value: rows.length,                                                gradient: 'from-[#1897C6] to-[#67BAC3]',  border: 'hover:border-[#1897C6]/50',   icon: UserMinus   },
          { label: 'Inactive', value: rows.filter(r => r.student.status === 'inactive').length,   gradient: 'from-[#1897C6] to-[#67BAC3]',  border: 'hover:border-[#67BAC3]/50',   icon: UserMinus   },
          { label: 'Blocked',  value: rows.filter(r => r.student.status === 'blocked').length,    gradient: 'from-red-400 to-red-500',      border: 'hover:border-red-400/50',     icon: AlertCircle },
          { label: 'Archived', value: rows.filter(r => r.student.status === 'archived').length,   gradient: 'from-orange-400 to-orange-500', border: 'hover:border-orange-400/50', icon: AlertCircle },
        ].map((card, i) => {
      ...
      <Card className="border-2">
        <CardHeader className="pb-3 sm:pb-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <CardTitle className="text-base sm:text-lg">Inactive Students List</CardTitle>
            <Button
              ...
              <Table>
                <TableHeader>
                  <TableRow className="bg-gradient-to-r from-[#1897C6]/5 to-[#67BAC3]/5 hover:from-[#1897C6]/5 hover:to-[#67BAC3]/5 border-b-2">
                    ...
                    <TableHead className="font-semibold text-sm h-14 hidden md:table-cell">Student Code</TableHead>
                    <TableHead className="font-semibold text-sm h-14 hidden md:table-cell">Contact Details</TableHead>
                    <TableHead className="font-semibold text-sm h-14 hidden lg:table-cell">Gender</TableHead>
                      ...
                      <TableCell colSpan={6} className="py-16 text-center">
                        <div className="flex flex-col items-center gap-3">
                          ...
                          <p className="text-base font-semibold">
                            {search ? 'No results found' : 'No inactive students'}
                          </p>
                          <p className="text-sm text-muted-foreground">
                            {search ? 'Try adjusting your search' : 'All students are currently active'}
                          </p>
                        </div>
                      </TableCell>
                    </TableRow>
                  ) : (
                    paginatedRows.map(({ student, contact }) => {
                      const initials =
                        ...
                        >
                          ...
                          <TableCell className="py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <p className="font-semibold text-sm text-gray-600">{student.full_name}</p>
                                <p className="text-xs text-muted-foreground md:hidden">
                                  {student.student_code || student._id.slice(-6)}
                                </p>
                              </div>
                            </div>
                          </TableCell>
                          ...
                          <TableCell className="hidden sm:table-cell py-4">
                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${
                              ...
                              student.status === 'blocked'  ? 'bg-red-100 text-red-600 border border-red-200'       :
                              student.status === 'archived' ? 'bg-orange-100 text-orange-600 border border-orange-200' :
                              'bg-gray-100 text-gray-500 border border-gray-200'
                            }`}>
                              {student.status === 'inactive' ? 'Inactive' :
                               student.status === 'blocked'  ? 'Blocked'  :
                               student.status === 'archived' ? 'Archived' : student.status}
                            </span>
                          </TableCell>
                          ...
                          <TableCell className="py-4">
                            <div className="flex items-center justify-end gap-1">
                              ...
                              <Button
                                variant="ghost" size="sm"
                                onClick={() => setReactivateId(student._id)}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This student will be moved back to the active list. All existing data is preserved.
              You may need to re-assign them to a class/batch after reactivation.
            </AlertDialogDescription>
            {actionError && (
